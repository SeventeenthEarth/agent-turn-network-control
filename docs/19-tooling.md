...
This document defines the core repository toolchain after the repo split. The core daemon and CLI are implemented in Go. The Python Hermes plugin tooling lives in `../../kkachi-agent-network-plugin/docs/04-tooling.md`.
...
The core repo docs must not reintroduce pre-split Python-core assumptions. Guardrails should reject stale wording that says the core is a Python package or that CLI/plugin share a Python client implementation. The valid split is: Go core, Python plugin, shared protocol contract, conformance tests.
