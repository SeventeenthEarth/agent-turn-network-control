...
`atn-control status <session_id> --verbose` includes the same runner accounting block (`runner_calls_total`, `missing_cost_runner_calls_total`, current token and USD totals, any unresolved budget block, and recent runner invocation failures), plus pending escalation batches and waiting-user state.
