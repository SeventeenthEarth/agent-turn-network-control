...
When agent-1's runtime observes the assignment, it first records `assignee_acknowledged` through `kkachi-agent-network delegate ack`.

When agent-1 later asks a clarification question through `kkachi-agent-network delegate clarify`,
then the daemon records `clarification_requested`.

When the moderator answers through `kkachi-agent-network delegate answer-clarification`,
...
When agent-1 submits work through `kkachi-agent-network delegate submit`,
...
When the Hermes plugin/gateway helper or equivalent moderator gateway skill reaches the user (via origin Hermes session, Telegram, Slack, or any other configured gateway) and records the result through a typed KAN command such as the canonical `kkachi-agent-network delegate escalation-delivered`,
...
When the Hermes plugin/gateway helper or equivalent moderator gateway skill fails to reach the requested target and records the failure through a typed KAN command such as the canonical `kkachi-agent-network delegate escalation-delivery-failed`,
...
When agent-2 returns a review verdict through `kkachi-agent-network delegate review-submit`,
then the daemon records `review_submitted` with the verdict.

When the verdict is `changes_requested`,
then the moderator can issue `kkachi-agent-network delegate revise`, which records `revision_requested`, and the session remains active.

## Scenario 4: Full council consensus path

Given no active session exists,
when the moderator creates a council with agent-1, agent-2, and agent-3 through `kkachi-agent-network council new`,
then the daemon records `session_created` and the council remains in `created`.

When the moderator starts preparation through `kkachi-agent-network council prepare`,
...
then each ready member records `member_ready` through `kkachi-agent-network council ready`, and the council enters discussion.

When member runtimes observe `hand_raise_requested`, raise hands through `kkachi-agent-network council hand-raise`, and the moderator selects speakers through `kkachi-agent-network council grant`,
then no speaker speaks twice in a row.

When the moderator proposes a conclusion through `kkachi-agent-network council propose` and requests a consensus vote through `kkachi-agent-network council request-vote`,
then the daemon records `consensus_vote_requested`, and member runtimes vote (`kkachi-agent-network council vote`) only after observing it.

When all members approve, the council finalizes (`kkachi-agent-network council finalize`) and transcript export includes all turns and votes.
...
If the member runtime itself records partial preparation before timeout through `kkachi-agent-network council prepared-partial`,
...
Given no active session exists,
when the moderator creates a council through `kkachi-agent-network council new` with `--surface discord-thread`, `--thread-id`, and `--kanban-card`,
...
when the moderator runs `kkachi-agent-network council prepare`,
...
When the moderator grants floor with `kkachi-agent-network council grant --mode role_order` or `--mode moderator_direct`,
...
when `kkachi-agent-network transcript <id> --format md` is run,
...
Given a valid artifact source path is submitted through `kkachi-agent-network delegate submit --artifact result.md`,
...
when the runtime disconnects and reconnects with `kkachi-agent-network stream <id> --member agent-1 --since cur_10 --follow`,
...
Given `kkachi-agent-network delegate message`,
then it must emit only `delegation_message` and must not emit `clarification_answered`.

Given `kkachi-agent-network delegate answer-clarification`,
...
Given `kkachi-agent-network council request-vote`,
...
then the emitted event is `user_escalation_requested` with `from: kkachi-agent-networkd` and a valid `causation_event_id`.
...
Given the assignee records `progress_status: blocked` through `kkachi-agent-network delegate update`,
...
Given the moderator records a manual block through `kkachi-agent-network delegate block`,
...
Given a registry defines `members.user` or `members.kkachi-agent-networkd`,
...
Given a delegation session is in `working`,
when the moderator records a manual block through `kkachi-agent-network block`,
...
when the moderator records `kkachi-agent-network resume --blocked-event <event_id>`,
...
when the moderator attempts `kkachi-agent-network resume`,
then the daemon rejects it and requires `kkachi-agent-network limits extend` with user authorization.

Given a council session is in `discussion`,
when the moderator records `kkachi-agent-network block`,
then the council enters `blocked` and still holds the active-session lock.

When the moderator records `kkachi-agent-network resume`,
then the council returns to the recorded `resume_phase`.

Given `kkachi-agent-network delegate block` is used in place of the common command for a delegation session,
...
then the event has `from: "kkachi-agent-networkd"` and `payload.member` contains the timed-out member id.

Given a member runtime explicitly records partial preparation through `kkachi-agent-network council prepared-partial`,
...
Given an `kkachi-agent-network resume` command targets a budget-originated block,
when the command is run with `--format json`,
then the CLI returns a JSON error envelope with category `session_lock`, a code identifying the budget mismatch, and `next` listing `kkachi-agent-network limits extend ...` instead of `kkachi-agent-network resume`.
