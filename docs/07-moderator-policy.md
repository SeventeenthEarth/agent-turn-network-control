...
The moderator and members operate through the `kkachi-agent-network` CLI. Long-lived runtimes observe `kkachi-agent-network stream` and write typed commands back to the daemon. The moderator must not treat fresh one-shot subprocess prompts as the primary council participation loop.
