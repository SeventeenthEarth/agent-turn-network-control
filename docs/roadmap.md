...
| Task ID | Task Title | Task Status | Task Description |
| --- | --- | --- | --- |
| RUNFIX2-001 | Evidence/config semantics and terminal readiness model | planned | Separate `*_enabled`/`*_required` production defaults from evidence-derived `*_pass` labels. Final/terminal reports must evaluate participant runtime readiness at grant/turn time and avoid treating naturally stale post-final heartbeat/ack as proof that the original discussion lacked prerequisites. |
| RUNFIX2-002 | Selected-runner Hermes adapter response-generation fix | planned | Replace the selected-runner adapter's delivery-only command mismatch with an explicit response-generation contract so `speaker_selected` can produce `runner_invocation_succeeded` and linked canonical `speech` for the selected member. Durable `runner_invocation_failed` remains terminal diagnostic evidence and cannot be masked by later fallback/manual speech. |
| RUNFIX2-003 | Discussion lifecycle closeout turns | planned | Change the discussion turn model so `max_discussion_turns` counts participant discussion turns only: T0 moderator opening, T1..Tmax selected participant discussion, one closeout turn per participant, and final moderator summary/conclusion. Total visible turns are `max_discussion_turns + participant_count + 2`. |
