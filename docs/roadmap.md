...
| RUNFIX-011 | Participant runtime readiness and attendance preflight | planned | Control implementation task to close the no-response gap observed in `sess_rename_hun_20260618T152054Z`: add or expose fail-closed live-council readiness evidence for required participant runtimes, including stream subscriber presence, cursor ack/heartbeat freshness, attendance auto-response or timeout recording, selected-speaker runner readiness prerequisites, and diagnostics proving that `gateway running` is not treated as `participant runtime ready`. This task includes control docs/status reconciliation for RUNFIX-010 PASS_WITH_RISK limits. It must not mutate live profiles, gateways, providers, auth, tokens, Discord state, or production daemons without separate explicit approval. |

Plugin-owned `plugin/RUNFIX-012` will consume the new control readiness evidence in activation planner/operator guardrails, and `plugin/RUNFIX-013` will close the KAN council moderation skill gap discovered by 주유's live moderation attempt. Those rows live in the plugin roadmap and the shared RUNFIX SOT tables; this control roadmap does not convert them into control implementation authority.
