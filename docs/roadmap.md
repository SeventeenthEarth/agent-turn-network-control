...
| Task ID | Task Title | Task Status | Task Description |
| --- | --- | --- | --- |
| HUN-001 | Control naming SOT and roadmap lock | completed/docs-only | Added `docs/26-hermes-unified-network-control-naming-sot.md` to lock the control-side public naming contract: product `Hermes Unified Network`, short label `HUN`, repo `hun-control`, CLI `hun`, daemon `hund`, data-home `HUN_HOME` / `~/.hun`, clean no-alias policy, control/plugin authority split, and downstream control task sequence. This task does not rename binaries, mutate runtime/profile/gateway/provider/auth/token state, publish packages, claim live readiness, or complete the repository-wide stale-reference sweep. |
| HUN-003 | Control code and binary rename | planned | Rename Go module, CLI binary, daemon binary, command help, Makefile surfaces, data-home/socket examples, and local smoke checks to the HUN control naming contract. |
| HUN-005 | Control protocol, fixtures, and tests rename | planned | Rename protocol docs, conformance manifests, fixtures, schemas, CLI tests, and plugin-contract checks to HUN labels only. |
| HUN-007 | HUN vanilla Hermes runtime hardening | planned | Reconfirm selected-runner evidence, wrapper identity, no role substitution, daemon authority, and fail-closed diagnostics under the HUN names. |
