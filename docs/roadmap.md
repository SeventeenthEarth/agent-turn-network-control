...
Post-feedback plan amendment: before `plugin/RUNFIX3-002` starts, RUNFIX3 freezes `selected_runner_pass` as an evidence-derived label rather than a daemon selection policy or `selection_mode`. Plugin guidance/schema tasks consume this frozen control evidence contract. Final RUNFIX3 completion requires a named closeout artifact, negative fail-closed matrix evidence, Red/Orange/Gray review, 마초 Blue synthesis, and 주군 approval.
...
| RUNFIX3-004 | Control live-council diagnostics and enforcement follow-up | planned | Implement diagnostics/enforcement against the frozen evidence-label policy: `selected_runner_pass` remains durable evidence, not daemon `selection_mode` or policy. Connect lifecycle accounting, selected-runner/delivery-evidence requirements, exact thread binding diagnostics, missing-stage preflight/status/finalization diagnostics, and fail-closed propose/finalize behavior with unresolved as the explicit terminal alternative. Verification must include `git diff --check`, `make docs-guardrails`, `make check-plugin-contract`, focused storage/daemon/protocol tests for lifecycle, selected-runner, delivery target, missing closeout, missing synthesis, and unresolved closeout, `make test-prepare`, and ATN Red/Orange/Gray review plus Blue synthesis. |
