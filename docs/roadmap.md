...
| RUNFIX-003 | Selected-speaker member runtime dispatch | planned | Wire automatic selected-speaker dispatch so `speaker_selected` can start the selected member runner path without a custom harness, preserving cursor/idempotency and durable failure evidence. Requires accepted RUNFIX-001/002 SOT gates. |
| RUNFIX-004 | Hermes adapter command contract and diagnostics | planned | Replace the incorrect platform-delivery command assumption with an explicit response-generation command contract, preserve redacted invocation diagnostics, and prevent stale runner phase evidence. Requires RUNFIX-003. |
| RUNFIX-005 | ARGUE and moderator quality gates | planned | Tighten hand-raise/speech relation quality rules, orphan-speech diagnostics, moderator scoring warnings, and lifecycle-vs-discussion-quality verdict separation. Requires RUNFIX-004. |
| RUNFIX-009 | Integrated control smoke fixtures | planned | Prove control-only runner invocation, canonical speech linkage, ARGUE quality diagnostics, and export/closeout evidence before the plugin live-local pilot. Requires plugin/RUNFIX-008. |
