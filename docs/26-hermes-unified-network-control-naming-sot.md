...
| HUN-014 | cross-repo | completed/local-compatibility-proof | Completed local compatibility proof under run `run-20260623T033911Z-267d9cd68d9c` closes final local/public naming compatibility for control/plugin paths, contract-check env precedence, docs-map status, and conformance labels. Live readiness, Discord delivery, hosted repository rename, package publication, profile/provider/gateway/auth/token mutation, push, and production rollout remain out of scope. |
