...
PRSLR is the eleven-task implementation epic for the SOT `17thHermes:40_outputs/team/macho/atn/2026-07-04-atn-persistent-participant-runtime-design-sot.md` plus the follow-on dialogue-quality hardening planned from the 2026-07-05 KLM token/speed final report (`17thHermes:40_outputs/team/jooyoo/atn/klm-token-speed-20260705T174553Z/final_report.md`). It repairs the context/latency bug by keeping one council-scoped participant session per member, delivering public speech as all-member cursor coverage, using first-class `council.drop`, closing response windows early when all members respond, and allowing only `delta` or `rehydrate` modes. Stateless fallback and repeated full-history hot prompts are rejected in persistent mode.
