...
PRSLR-008 through PRSLR-011 are the dialogue-quality extension before final PRSLR epic close. `control/PRSLR-008` has completed bounded local final verification, improve closeout, and approved local commit under KAH run `run-20260705T191135Z-83a7a11db6cf`: local implementation now makes selected-runner success labels require `runner_invocation_succeeded` plus linked canonical `speech` for the same selected member and positive selected turn, while fallback/manual, runnerless, wrong-turn, and missing-selected-turn speech remains diagnostic-only. MAR recovery rerun-1 passed and KAH `mar_review` completed at `evt-003949`. The prior post-MAR Orange/Gray status-packet blockers were remediated by refreshed checklist/roadmap/live-transport surfaces; focused Orange `t_7daa51ed` and Gray `t_e6d6decb` returned `APPROVE_WITH_FINDINGS`, Blue accepted the remediation in `.kkachi/runs/run-20260705T191135Z-83a7a11db6cf/second-color-focused-rereview-blue-synthesis.md`, and KAH `second_color_review` completed at `evt-003956`, workflow revision `23`. `control/PRSLR-009` remains the next planned daemon quality-contract task for `prior_speaker_argue_quality_evidence`: every non-opening quality-required speech must target at least one previous speaker or claim through valid `stance_links[]`; `responds_to_event_id`, prose references, Discord order, and moderator summaries are display/audit hints only. `plugin/PRSLR-010` updates participant/moderator guidance and local validation to produce those links without owning daemon quality state. `cross-repo/PRSLR-011` proves the end-to-end discussion-quality path and updates closeout evidence without live-readiness or production claims. PRSLR-008 final verification is locally passed by `.kkachi/runs/run-20260705T191135Z-83a7a11db6cf/final-report.md` and `verification.md`, and improve closeout is recorded in `.kkachi/runs/run-20260705T191135Z-83a7a11db6cf/improvement-note.md`. This is local/default-off completion only; push, live/runtime activation, Discord delivery, production readiness, provider/profile/gateway/auth/token/model mutation, package publication, broad rollout, and cross-repo PRSLR epic completion remain unclaimed.
