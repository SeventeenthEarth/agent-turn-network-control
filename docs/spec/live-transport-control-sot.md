...
- `control/NEWFIX-005` now makes Discord live-visible selected-runner timeout readiness explicit: `dispatch_timeout_sec=150` or an explicitly approved alternative is required/evidenced for that path, typed `selected_runner_timeout_evidence` is visible through council status and `bundle_manifest.json`, later daemon-wide timeout drift fails closed, and `stream.status` remains unchanged.
...
PRSLR is the fourteen-task cross-repo/direct implementation epic for the SOT `17thHermes:40_outputs/team/macho/atn/2026-07-04-atn-persistent-participant-runtime-design-sot.md` plus the follow-on dialogue-quality hardening planned from the 2026-07-05 KLM token/speed final report (`17thHermes:40_outputs/team/jooyoo/atn/klm-token-speed-20260705T174553Z/final_report.md`) and the direct post-live-audit hardening follow-ups requested after the 2026-07-07 live KLM audits. It repairs the context/latency bug by keeping one council-scoped participant session per member, delivering public speech as all-member cursor coverage, using first-class `council.drop`, closing response windows early when all members respond, and allowing only `delta` or `rehydrate` modes. Stateless fallback and repeated full-history hot prompts are rejected in persistent mode. PRSLR-012 adds visible-delivery echo de-duplication as control-owned accounting; PRSLR-013 adds selected-runner claim-kind prompt/schema guidance, visible echo linkage guidance, and closeout lifecycle completion guidance across control/plugin surfaces. PRSLR-014 raises Discord live-visible selected-runner timeout policy/evidence to 150 seconds, rejects runnerless visible-delivery echo re-recording after posted selected-runner canonical speech at the control boundary, and mirrors timeout/closeout guidance in plugin operator surfaces.

Control owns the authoritative participant-session registry, response window, cursor coverage, rehydrate/fail-close decisions, `persistent_participant_runtime_evidence` production, and planned `prior_speaker_argue_quality_evidence` production. Plugin surfaces may consume that evidence but must not invent lifecycle, cursor, lock, dedupe, recovery, or fallback semantics. PRSLR remains default-off/local-only until its KAH/KAS implementation, review, MAR, second-color, final verification, and explicit post-epic activation gates complete; no live readiness or production activation is claimed. Participant sessions must not be reused across councils. Rehydrate/observe/ack failure is not a drop; default recovery failure scope is whole participant runtime/council blocked, and affected-member degraded continuation is out of scope unless later explicitly approved. Plugin `PRSLR-006` completed local/default-off proof in plugin KAH run `run-20260705T094456Z-e71c7b7d9795`: the first provider MAR summary was `DEGRADED / unresolved_required_role_coverage`, Red adjudication `t_06900c3c` and Blue synthesis `t_3933d063` authorized strict rerun, the strict rerun passed with all required roles covered, KAH `mar_review` completed at `evt-003350` after run-local state repair `evt-003349`, official second-color review completed at plugin `evt-003354` with Red `t_84bd14ba`, Orange `t_c01cc684`, Gray `t_12e94e1a`, and Blue synthesis `t_a78ee68b` accepting final_verify routing with carry-forwards, KAH `final_verify` completed at `evt-003356`, and KAH `improve` completed at `evt-003358`. Control records this as dependency/mirror status only and does not claim plugin runtime activation, commit/push, live readiness, provider/profile/gateway/auth/token/model mutation, or cross-repo PRSLR acceptance. `PRSLR-007` completed local/default-off proof in primary control KAH run `run-20260705T132654Z-ae2091713390`: plan-vet accepted with carry-forwards, ask approval was recorded, KAH-wrapped GJC ULTRAGOAL reached `ultragoal_ready`, and local/default-off proof artifacts under `artifacts/acceptance/` cover the acceptance matrix, control-produced persistent participant runtime evidence, plugin consumption boundary, and operator report. Full local verification passed across control and plugin. Post-MAR second-color passed after focused Gray remediation and Blue synthesis; KAH `second_color_review` completed at `evt-003903`, KAH `final_verify` completed at `evt-003905`, and KAH `improve` completed at `evt-003907`, workflow revision 27. Commit/push, live/runtime activation, Discord delivery, production readiness, provider/profile/gateway/auth/token/model mutation, and post-PRSLR activation remain unclaimed.
...
PRSLR-008 through PRSLR-011 are the dialogue-quality extension that preceded the later direct PRSLR-012/PRSLR-013/PRSLR-014 live-audit hardening follow-ups. `control/PRSLR-008` has completed bounded local final verification, improve closeout, and approved local commit under KAH run `run-20260705T191135Z-83a7a11db6cf`: local implementation now makes selected-runner success labels require `runner_invocation_succeeded` plus linked canonical `speech` for the same selected member and positive selected turn, while fallback/manual, runnerless, wrong-turn, and missing-selected-turn speech remains diagnostic-only. MAR recovery rerun-1 passed and KAH `mar_review` completed at `evt-003949`. The prior post-MAR Orange/Gray status-packet blockers were remediated by refreshed checklist/roadmap/live-transport surfaces; focused Orange `t_7daa51ed` and Gray `t_e6d6decb` returned `APPROVE_WITH_FINDINGS`, Blue accepted the remediation in `.kkachi/runs/run-20260705T191135Z-83a7a11db6cf/second-color-focused-rereview-blue-synthesis.md`, and KAH `second_color_review` completed at `evt-003956`, workflow revision `23`. `control/PRSLR-009` has completed bounded local proof under KAH run `run-20260706T092628Z-8b113d0833db`: classification, CodeGraph refresh, KAH-wrapped GJC RALPLAN receipt evidence, official plan-vet, explicit ask approval, KAH-wrapped GJC ULTRAGOAL setup, implementation, enhance-test, optimize, docs update, official implementation review, no-source-change handle-feedback, MAR provider/script review, post-MAR second-color review, final verification, and improve closeout are recorded. Current local tests prove `quality_required` rejects the first non-opening orphan participant speech even if `responds_to_event_id` points at a prior event; status/execution surfaces expose `prior_speaker_argue_quality_evidence` with valid prior speech/claim targets, first orphan event, display-hint rejection, and justified `new_axis` exceptions. MAR merge-pack status is `PASS` with all required roles covered, `unresolved_required_roles=[]`, no Red adjudication trigger, watcher retirement evidence, and Blue disposition `APPROVE_WITH_FINDINGS / ATN_BLUE_PRSLR009_MAR_ACCEPTED_FOR_SECOND_COLOR`; KAH `mar_review` completed at `evt-003995`. Initial post-MAR Orange/Gray status-surface blockers were remediated through focused re-review and Blue synthesis; KAH `second_color_review` completed at `evt-004002`. Final verification passed local control/plugin checks and completed at `evt-004007`; KAH `improve` completed at `evt-004009`, workflow revision `27`. The contract remains: every non-opening quality-required speech must target at least one previous speaker through structured `stance_links[]` to prior speech event ids and, where applicable, claim ids; legacy display hints remain non-authoritative. `plugin/PRSLR-010` has completed task classification, CodeGraph refresh, KAH-wrapped GJC RALPLAN receipt, KAH `plan`, official Red/Orange/Gray plan-vet, Blue synthesis `ATN_BLUE_PRSLR010_PLAN_VET_ACCEPTED_WITH_CARRY_FORWARDS`, explicit ask approval, implementation, feedback handling, strict MAR rerun, and Blue MAR disposition in plugin KAH run `run-20260706T120949Z-6faa592a3ca8`; plugin KAH `second_color_review` completed at `evt-003397`, `final_verify` completed at `evt-003399`, `improve` completed at `evt-003401`, and PRSLR-010 local proof is completed/local-proof-committed via plugin commit `ce988e2` and control mirror commit `fc673b4`. Control records this as dependency status only; push, live/runtime activation, Discord delivery, production readiness, provider/profile/gateway/auth/token/model mutation, broad rollout, and PRSLR epic completion remain unclaimed. PRSLR-011 local/default-off final verification and improve/status closeout are summarized in the dedicated PRSLR-011 paragraph below; push, live/runtime activation, Discord delivery, and production readiness remain unclaimed.

PRSLR-011 implementation proof is now in bounded local/default-off completed/local-proof-committed state under primary control KAH run `run-20260706T144956Z-c4f49751f7f4`: request_feedback completed at `evt-004045`, focused Orange status-artifact remediation was accepted, handle_feedback completed at `evt-004047`, MAR completed at `evt-004052`, `second_color_review` started at `evt-004053`, and `second_color_review` completed at `evt-004059` after Red `t_529c1666`, Orange `t_019580e4`, Gray `t_776e2b69`, focused Orange `t_230f3cb0`, focused Gray `t_102d90f9`, and dependent Blue `t_5564ae23`. The implementation artifact `internal/daemon/prslr011_e2e_test.go` exercises two selected participant runner invocations through the daemon selected-runner path, verifies linked canonical `speech` events and `selected_runner_accounting.selected_runner_pass=true`, requires structured `stance_links[]` support from the non-opening speech to the prior speaker claim, checks `prior_speaker_argue_quality_evidence.pass=true`, and preserves moderator synthesis plus transcript/export/projection evidence rows. The fixture uses local/fake evidence only and remains non-live: it does not prove Discord delivery, runtime activation, production readiness, profile/provider/gateway/auth/token/model mutation, package publication, push, or broad rollout; final verification completed at `evt-004062`, improve/status closeout completed at `evt-004064`, workflow revision `27`, and no ready nodes remain.
