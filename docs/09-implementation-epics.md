...
| Phase 14 | RUNFIX | council runner, activation, and discussion-quality remediation | RUNFIX-001/RUNFIX-002 docs-only SOT locks are accepted after Red/Orange/Gray review, focused re-check, and Blue final synthesis; RUNFIX-003 through RUNFIX-010 are completed within their repo-qualified bounded scopes, with RUNFIX-010 classified as PASS_WITH_RISK rather than production/live readiness |
...
| 11 | control | RUNFIX-011 | Planned: implement participant runtime readiness and attendance/preparation preflight so a Discord-origin council cannot silently stall at `attendance_requested` with no subscribers/cursors. | `internal/daemon/`, `internal/memberruntime/`, stream/status/projection diagnostics, control RUNFIX docs | prove subscriber/ack/heartbeat readiness or fail closed; attendance auto-response or timeout records; selected-speaker readiness prerequisites; no gateway/profile/provider/auth/token mutation |
...
RUNFIX implementation tasks after accepted RUNFIX-001/RUNFIX-002 docs-only SOT locks require separate 주군 authorization before implementation starts. `RUNFIX-003` through `RUNFIX-010` are completed within their repo-qualified bounded scopes; `RUNFIX-011` through `RUNFIX-013` are planned follow-up tasks opened from the 2026-06-19 no-response and moderation-skill-gap evidence. Completed proofs still do not authorize production activation, live readiness, broad rollout, or profile/provider/gateway/auth/token/model mutation. `plugin/RUNFIX-010` completed with bounded PASS_WITH_RISK visible-local pilot evidence in KAH run `run-20260618T112843Z-40b023a5d9c8` and now includes the live-visible/default-output guardrail; remaining limitations are parent-channel fallback is not no-restart thread readiness, `real_profile_live_invocation=false`, full KAN team roster coverage is unproven, and participant runtime subscription/attendance readiness remains owned by `control/RUNFIX-011`.
