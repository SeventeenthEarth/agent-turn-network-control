...
| 12 | plugin | RUNFIX-012 | Planned: consume control readiness evidence in the plugin activation planner/operator guardrails and reconcile plugin RUNFIX docs/status wording. | plugin `activation_planner`, schemas, operator guide, bundled skill, plugin RUNFIX SOT | `live_readiness=false` unless runtime subscriber/cursor/attendance/selected-runner/visible-surface evidence is explicit; no transcript/export-only or gateway-only success promotion |
...
RUNFIX implementation tasks after accepted RUNFIX-001/RUNFIX-002 docs-only SOT locks require separate 주군 authorization before implementation starts. `RUNFIX-003` through `RUNFIX-011` are completed within their repo-qualified bounded local/control scopes; `RUNFIX-012` through `RUNFIX-013` are planned follow-up tasks opened from the 2026-06-19 no-response and moderation-skill-gap evidence. Completed proofs still do not authorize production activation, live readiness, broad rollout, or profile/provider/gateway/auth/token/model mutation. `plugin/RUNFIX-010` completed with bounded PASS_WITH_RISK visible-local pilot evidence in KAH run `run-20260618T112843Z-40b023a5d9c8` and now includes the live-visible/default-output guardrail; remaining limitations are parent-channel fallback is not no-restart thread readiness, `real_profile_live_invocation=false`, full KAN team roster coverage is unproven, and plugin/RUNFIX-012 must consume `control/RUNFIX-011` readiness evidence before plugin/operator live-readiness wording can change.
