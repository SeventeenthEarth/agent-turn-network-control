...
| 14 | control | RUNFIX-014 | Planned: selected-runner terminal accounting and live-report guard from 주유's 2026-06-19 rename-council harness feedback. Final reports must distinguish lifecycle success from selected-speaker runner success when `runner_invocation_failed` appears before a later canonical `speech`, and must expose runner started/succeeded/failed counts plus fallback/manual harness use before any live-readiness or automatic-runner claim. | `internal/runner/`, `internal/daemon/`, status/transcript/export/report helpers | tests for started/failed/succeeded runner accounting, canonical speech linkage, and fail-closed live-readiness labels; no profile/provider/gateway/auth/token mutation or Discord delivery claim |
