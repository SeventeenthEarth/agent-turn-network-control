...
| 16 | control | RUNFIX-016 | Planned: canonical summary and turn-accounting helper for `channel.jsonl` / export bundles. The helper must tolerate `payload.plugin_evidence` dict, `payload.evidence` dict, `payload.evidence` list, and missing optional evidence without failing after `council_finalized`, and should emit stable turn/member/selected/speech/runner/visible-message fields. | CLI/export/transcript helpers, storage/report tests | regression fixtures for dict/list/missing evidence; finalized lifecycle must not be reported failed due to optional evidence shape |
