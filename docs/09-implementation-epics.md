...
| 15 | plugin | RUNFIX-015 | Planned: visible author guard for live-visible councils. The operator harness/plugin guidance must probe the same outbound Discord posting path per selected profile, record `profile`, expected author source (`registry_snapshot` or explicit approved profile-author map), `bot_id`, `username`, `source_env`, `posting_path`, and fail closed before `council.new` if any selected participant resolves to a shared/default or unexpected author. | plugin operator guide/bundled skill; visible runner/harness docs | profile env precedence test: shared defaults first, profile-local env overrides; per-turn message id/member/author/speech linkage evidence; no hidden fallback |
