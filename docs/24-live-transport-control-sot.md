...
- `control/NEWFIX-005` must make Discord live-visible selected-runner timeout readiness explicit: `dispatch_timeout_sec=120` or an explicitly approved alternative is required/evidenced for that path, effective timeout evidence must be visible, and missing timeout must not silently fall back to 30 seconds for start-capable live-visible sessions.
...
| 4 | control | NEWFIX-004 | planned | Control must implement selected-member own-history prompt context and prompt evidence source-id separation so participant continuity does not depend on persistent Hermes session memory. |
| 5 | control | NEWFIX-005 | planned | Control must require/evidence `dispatch_timeout_sec=120` or an approved alternative for Discord live-visible selected-runner councils, expose effective timeout evidence, and prevent silent 30-second fallback for that path. |
| 6 | plugin | NEWFIX-006 | planned | Plugin must consume `NEWFIX-004` prompt own-history evidence and `NEWFIX-005` timeout evidence in activation planning/operator guidance and keep local-only/artifact/manual bridge paths non-equivalent. |
