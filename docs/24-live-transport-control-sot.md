...
| 2 | plugin | RUNFIX3-002 | implementation_complete/review_pending | Plugin-owned moderator/operator guidance hardening against the frozen control evidence contract is implemented and awaiting focused reviewer acceptance after cross-repo mirror/status reconciliation. Fixed minimum gates include plugin docs guardrails, core contract, test-prepare, focused bundled-skill/guardrail/load-smoke tests, Red/Orange/Gray review, and Blue synthesis. |
