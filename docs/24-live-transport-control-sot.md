...
| 14 | control | RUNFIX-014 | planned | Selected-runner terminal accounting and report guard from 주유's `sess_kan_rename_hun2_20260619T033549Z` feedback. A run with `runner_invocation_started` and `runner_invocation_failed` before later canonical `speech` is lifecycle/fallback evidence, not automatic selected-runner success. Control must expose started/succeeded/failed runner counts, canonical speech linkage, and fallback/manual harness flags in status/transcript/export summaries before any selected-runner or live-readiness claim. |
| 15 | plugin | RUNFIX-015 | planned | Visible author guard for live-visible councils. Plugin/operator harness guidance must require same-path per-profile Discord identity probes, expected author source (`registry_snapshot` or explicit approved profile-author map), source-env/posting-path evidence, env precedence proof, per-turn Discord message id/member/author/speech linkage, and fail-closed behavior before `council.new` when a selected participant resolves to a shared/default or unexpected author. |
| 16 | control | RUNFIX-016 | planned | Canonical summary/turn-accounting helper for `channel.jsonl` and export bundles. Control should parse `payload.plugin_evidence`, dict/list `payload.evidence`, and missing optional evidence without post-finalization crashes; output stable turn, member, `speaker_selected_event_id`, `speech_event_id`, `runner_invocation_id`, and visible message id where available. |
| 17 | plugin | RUNFIX-017 | planned | ARGUE quality-required prompt and runner contract hardening. Plugin/operator guidance must provide compact prior claim graph targets to selected participants, preserve `claims[]`, `stance_links[]`, `contribution_type`, `new_axis_reason`, and `evidence[]`, and fail `discussion_quality_pass` on the first orphan non-opening speech in `quality_required` pilots and expose repeated-orphan counts without synthetic relation inference while relying on control/RUNFIX-005 quality diagnostics. |
