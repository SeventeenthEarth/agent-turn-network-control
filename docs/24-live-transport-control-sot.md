...
| 11 | control | RUNFIX-011 | planned | Implement participant runtime readiness and attendance/preparation preflight after `sess_rename_hun_20260618T152054Z` showed `attendance_requested` with no member cursors/subscribers. Control must expose or enforce required-member runtime subscription, cursor ack/heartbeat freshness, attendance auto-response or timeout records, and selected-speaker runner readiness prerequisites before live-visible councils can proceed. |
| 12 | plugin | RUNFIX-012 | planned | Consume `control/RUNFIX-011` readiness evidence in activation planner/operator guardrails and reconcile plugin RUNFIX docs so gateway liveness, transcript/export artifacts, and parent-channel visible fallback cannot be reported as participant-runtime/live discussion readiness. |
| 13 | plugin | RUNFIX-013 | planned | Add council moderation skill hard rules from 주유's moderation-skill-gap report: lifecycle first, no hard-coded full speaker order, per-turn poll/hand-raise evaluation, justified `speaker_selected`, daemon `speech` event authority, moderator-opinion handling, and cancel/restart versus repair-forward guidance. |

Control-owned RUNFIX implementation must fail closed on missing selected-speaker dispatch evidence, missing participant runtime subscriber/ack/heartbeat readiness, adapter response-generation mismatch, registry/profile identity mismatch, cursor gap, orphan speech in quality-required mode, stale runner phase, or fallback-only evidence mislabeled as KAN success.
