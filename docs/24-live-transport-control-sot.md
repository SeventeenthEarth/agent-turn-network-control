...
`NEWFIX-008` and `NEWFIX-009` remain planned follow-ups for the plugin representation boundary and cross-repo agenda-context acceptance proof.
...
- `control/NEWFIX-007` repairs the operator CLI/conformance path so `success_criteria` can be written as structured `agenda_locked.payload.success_criteria`, defines agenda JSON file input as explicit/fail-closed structured input, and rejects storing agenda JSON as a `draft` string selected-runner context path. Its implementation, request-feedback, handle-feedback, and MAR gates are complete; second-color review is still pending.
- `plugin/NEWFIX-008` must keep daemon `command_id` authority explicit when submitting structured `council.lock_agenda` payloads; plugin `idempotency_key` compatibility cannot become hidden local dedupe, hidden ID generation, or CLI fallback.
...
| 7 | control | NEWFIX-007 | completed | Control has repaired agenda-lock required-context submission by requiring structured `decision_question`, `success_criteria`, and `out_of_scope_policy` at the CLI and daemon/storage boundary, aligning CLI/spec/conformance fixtures, and making agenda file input explicit/fail-closed rather than `draft`-string fallback. Request-feedback and handle-feedback gates are accepted; MAR completed after focused arch re-review resolved the prior status-surface coherence blocker and Blue disposition allowed KAH `mar_review` completion. Second-color review, final verification, and improve/lessons gates are complete. This row does not claim live readiness, runtime activation, commit, or push. |
| 8 | plugin | NEWFIX-008 | planned | Plugin must repair or clarify live `command.submit` representation for structured `council.lock_agenda` payloads, preserving daemon `command_id` authority and rejecting hidden CLI fallback or local ID generation. |
| 9 | cross-repo | NEWFIX-009 | planned | Cross-repo acceptance must prove `decision_question`, `success_criteria`, and `out_of_scope_policy` reach agenda payload and selected-runner prompt evidence; the cancelled `sess_council_1782671073534714000` remains diagnostic defect evidence only. |
