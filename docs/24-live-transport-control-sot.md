...
Legacy `kan-*`, `kan_discussion_*`, `sess_kan_*`, and `kan/` artifact path fragments in older RUNFIX rows are historical/provenance labels from pre-ATN evidence. They are not current public ATN aliases.
