...
- CLI itself never spawns shells; the Hermes plugin must not spawn shells for normal ATN state mutations; only the daemon runner invokes subprocesses, and only through the adapter interface defined in `13-operational-contracts.md`.
